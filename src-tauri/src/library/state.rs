...
use crate::library::settings::LibrarySettings;
...
pub struct LibraryState {
    pub db: Arc<Database>,
    pub settings: Arc<RwLock<LibrarySettings>>,
...
impl LibraryState {
    pub fn new(app_handle: AppHandle) -> AppResult<Self> {
        ...
        let settings_path = app_data_dir.join("library_settings.json");
        let settings = LibrarySettings::load(&settings_path)?;
    ...
    pub fn thumbnail_dir(&self) -> AppResult<std::path::PathBuf> {
        ...
        let thumb_dir = app_data_dir.join("library_thumbnails");
