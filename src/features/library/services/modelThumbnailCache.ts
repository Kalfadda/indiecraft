...
async function loadModel(filePath: string, extension: string): Promise<THREE.Object3D | null> {
  ...
  return new Promise((resolve) => {
    ...
    // Add timeout to prevent hanging on large/corrupt files
    const timeout = setTimeout(() => {
      console.warn('Model load timeout:', filePath);
      resolve(null);
...
export async function generateAllModelThumbnails(
  ...
  for (const asset of assets) {
    try {
      ...
      const result = await renderModelToThumbnail(asset.absolute_path, asset.extension);
      if (result) {
      ...
      memoryCache.set(getCacheKey(asset.id, asset.modified_time), null);
    }
